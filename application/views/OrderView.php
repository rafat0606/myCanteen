...
foreach ($ords as $rec) {
	//print_r($rec);
	// foreach($rec as $reca)
	echo " <form method=\"post\" class=\"text\" action=\"http://localhost/CanteenApp/index.php/OrdersController/delete\">";
	echo "Order Id:"  .$rec->orderId."<input type=\"hidden\" name=\" id\" value="  .$rec->orderId."/><br>User Id:"  .$rec->userId."<br>Item Id:   ".$rec->itemId."&nbsp &nbspName:"  .$rec->itemName."&nbsp &nbspQuantity:"  .$rec->quantity."<br>Total Bill:"  .$rec->totalBill."</br> <input type=\"submit\" name=\"accept\" class=\"add\" value=\"accept\">             <input type=\"submit\"  class=\"add\" name=\"reject\" value=\"reject\"></br>";
